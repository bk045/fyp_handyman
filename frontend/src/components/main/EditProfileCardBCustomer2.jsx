...
const EditProfileCardBCustomer
 = () => {
    ...
    const {obj:record} = location.state
    console.log("Record",record)
    const initialValues = {
        name_of_business:record.name_of_business,
        person_to_contact:record.person_to_contact,
        email:record.email, 
        ...
        document_photo: record.document_photo,
    ...
    const onSubmit = async (values)=>{
        if(record.profile_status==="Pending" && values.type_of_business_menu){
        ...
        if ((documentType==='string'|| userData.profile.document_photo === null)&&(profileType==='string'|| userData.profile.profile_picture === null)){
            ...
            await updateProfile(record.user_id, userProfile)  
        }
            ...
            await updateProfile(record.user_id, fData)
        }
        else if ((documentType !=='string'|| userData.profile.document_photo !== null)&&(profileType ==='string'|| userData.profile.profile_picture === null)){
            let userProfile =  {...userData.profile}
            delete userProfile['profile_picture']
            ...
            return <Form>
                <div className="outlet-container profile-container">
                    <div className="profile-wrapper shadow-myBoxShadow">
                        <div className="left">
                            <img src={record.profile_picture} alt="Profile Picture" height="200px" width="200px"/>
                            ...
                            <h4>{record.name_of_business}</h4>
                            <p>{record.type_of_business}</p>
                        ...
                        <div className="top-bar">
                            ...
                            <div className="document">
                                ...
                                <div className="document_data">
                                    <div className="data">
                                        <FormikControl control='input' type='text' name='registration_no' label='Registartion No' value={profile.registration_no} disabled={info.profile_status!=='Pending' || !editMode}/>
                                    </div>
                                    <div className="data">
                                            <label htmlFor="document_photo">Document Photo</label>
                                    {(record.profile_status === 'Pending' && editMode) && <div>
                                            <input type='file' name='document_photo' label='Document Photo' onChange={(e)=>formik.setFieldValue('document_photo', e.target.files[0])}/>
                                            <div className="text-red-700">
                                                <ErrorMessage name='document_photo'></ErrorMessage>
                                            </div>
                                        </div>}
                                        {record.document_photo && <img src={record.document_photo} alt="Doc" height="100px" width="100px"/>}
                                    </div>
                                </div>
                            </div>
                        </div>
